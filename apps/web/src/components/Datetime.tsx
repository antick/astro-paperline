...
export default function Datetime({ pubDatetime, modDatetime, size = 'sm', className }: Props) {
  return (
    <div className={`flex items-center space-x-2 ${className}`}>
      ...
      {hasUpdatedDate(pubDatetime, modDatetime) && modDatetime ? (
...
const FormattedDatetime = ({ pubDatetime, modDatetime }: DatetimesProps) => {
  const myDatetime = new Date(
    hasUpdatedDate(pubDatetime, modDatetime) && modDatetime ? modDatetime : pubDatetime
  );

  return <time dateTime={myDatetime.toISOString()}>{getFormattedDate(myDatetime)}</time>;
};
